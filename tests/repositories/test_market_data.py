...
class TestGetPriceData:
  ...
  def test_get_price_dataは取得したDataFrameを返す(self, mock_download):
  ...
  @patch("app.repositories.market_data.yf.download")
  def test_get_price_dataは正しい引数で実行される(self, mock_download):
    mock_download.return_value = dummy_df
  ...
  @patch("app.repositories.market_data.yf.download")
  def test_get_price_dataは例外が出た時エラーを返す(self, mock_download):
